turtles-own[
  velocity flockingmates colisionmates  ;;速度矢量, 在靠近范围内的所有邻居, 在分离邻居内的所有邻居
]
to setup
  clear-all
  crt num_boids[
    setxy random-xcor random-ycor ;;随机初始化位置
    set velocity [0 0] ;;初始化速度矢量为0
    set size 10 ;;鸟身体的大小
  ]
end
to go
  ask turtles[
    get-neighbors  ;;获取每只鸟的邻居，包括靠近邻居和分离邻居
...
to get-neighbors
  set flockingmates (other turtles in-radius viewdistance) ;; 获得所有viewdistance距离内的所有的其它鸟（靠近邻居）
  set colisionmates (other turtles in-radius collisiondistance) ;; 获得所有collisiondistance距离内的所有的其它鸟（分离邻居）
end
to flocking
  ...
  if (norm velocity) > 5 [
    ...
    set velocity map [ ?1 -> ?1 * 5 ] velocity ;;velocity = velocity * 5
  ]
