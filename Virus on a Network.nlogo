...
to add-hyper-links
  ;增加远程连接，并且按照偏好依附规则
  ask n-of num-of-hyper-links turtles[
    let attached [one-of both-ends] of one-of links
    if not link-neighbor? attached and not (self = attached)[
...
to go
  ...
  ;开始病毒传播
  ask turtles with [state = 1]
    [ ask link-neighbors with [state = 0]
...
"" ""
PENS
"susceptible" 1.0 0 -13345367 true "" "plot (count turtles with [state = 0]) / (count turtles) * 100"
"infected" 1.0 0 -2674135 true "" "plot (count turtles with [state = 1]) / (count turtles) * 100"
"recovery" 1.0 0 -7500403 true "" "plot (count turtles with [state = 2]) / (count turtles) * 100"
